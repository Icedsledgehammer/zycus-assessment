...
This repository contains my submission for the US Delivery Internship technical task. It uses the supplied synthetic support-ticket data, customer-account data, and product knowledge base to build two related support operations tools and an evaluation harness for checking them.
...
- **Task 3** evaluates the outputs from Task 1 and Task 2 using predefined cases, deterministic checks, scoring, and an optional independent Gemini review.

Tasks 1 and 2 expose FastAPI endpoints and use a local Ollama model where natural-language interpretation is useful. Task 3 runs those two implementations directly and produces evaluation reports. The parts that need to be exact, traceable, and reproducible are kept in Python rather than delegated to a model.
...
Task-3/
  eval_cases.py                   # Predefined Task 1 and Task 2 cases
  evaluators.py                   # Deterministic output checks
  scoring.py                      # Per-case and aggregate scoring
  llm_judge.py                    # Optional Gemini quality judge
  report.py                       # JSON and Markdown report generation
  runner.py                       # Evaluation entry point
  tests/                          # Evaluation harness tests
...
Task 3 provides a repeatable way to evaluate the systems built in Task 1 and Task 2. It is not another API. It initializes the existing implementations, runs a fixed set of evaluation cases, checks their outputs against explicit expectations, calculates scores, and writes a report.

The harness is useful because the two earlier tasks contain language-model steps whose output can vary. Task 3 separates objective requirements from qualitative review. Labels, required fields, account health values, risk types, and data-quality warnings are checked in Python. A separate Gemini call can then give an additional opinion about factual correctness, grounding, completeness, and usefulness.

### Evaluation cases

`Task-3/eval_cases.py` loads the supplied tickets and accounts and builds ten cases:

- `T1-01` to `T1-05` cover a feature request without relevant knowledge-base information, authentication or SSO, a bug report, a feature/performance request, and a billing ticket.
- `T2-01` to `T2-05` cover a healthy and growing account, an at-risk inactive account with churn signals, a churning account with declining usage, a healthy account with high support volume, and an account containing a deliberate source-data inconsistency.

Task 1 cases use real ticket records selected by ticket ID. Their expected values can include product area, issue category, urgency, knowledge-base match status, and other output fields. Task 2 cases use an `account_id` and check account health, usage trend, expected risks, required evidence, data-quality warnings, and whether recommended actions are present.

### Running a case

`runner.py` initializes Task 1 by loading the knowledge base and creating `TicketTriager`. It initializes Task 2 by creating `AccountDataLoader`, `RiskDetector`, and `TAMSummarizer`. The runner adds the Task 1 and Task 2 directories to `sys.path` so it can import their local modules without changing those tasks.

For each case, the runner calls the matching system, evaluates the returned dictionary, calculates a deterministic score, and stores the original output. An exception in one case is captured as an evaluation error and does not prevent the remaining cases from being attempted.

### Deterministic evaluation and scoring

`Task-3/evaluators.py` contains separate evaluators for the two output formats. Task 1 checks only the expectations declared by the case and also checks that the core required structure is present. Task 2 checks nested account-health fields, the presence of expected ticket IDs or risk types where requested, non-empty evidence quotes for relevant ticket risks, data-quality warnings, recommended actions, and the required top-level structure.

`Task-3/scoring.py` gives every check equal weight. A case score is the number of passed checks divided by the total number of checks, rounded to three decimal places. A case is marked `passed` only when its score is exactly `1.0`. The summary reports total cases, passed cases, failed cases, and the average deterministic score.

### Gemini judge

`Task-3/llm_judge.py` is an optional qualitative evaluator. It reads `GEMINI_API_KEY` and `GEMINI_MODEL` from the Task-3 `.env` file, defaulting to `gemini-2.5-flash-lite` when no model is specified. The judge receives the evaluation case and system output, then must return JSON containing a score from `0.0` to `1.0`, a `PASS` or `FAIL` verdict, and a short reason.

The judge result is supplementary. It does not replace the deterministic score. Invalid judge JSON, an invalid score, or a missing API configuration is recorded as a judge error for that case while the deterministic result remains available.

Do not commit `.env` or API keys. The repository's `.gitignore` excludes `.env`; create a local file in `Task-3` with the required variable when the Gemini review is needed:
...
`Task-3/report.py` writes two files under `Task-3/reports`:

- `evaluation_report.json` contains the aggregate summary and detailed result objects, including system outputs, checks, scores, errors, and judge results.
- `evaluation_report.md` contains a readable summary table, per-case results, failed checks, judge reasoning, execution errors, and the evaluation methodology.

The currently stored report contains 10 cases, 3 deterministic passes, 7 deterministic failures, and an average deterministic score of `0.680`. That snapshot is an evaluation result, not a claim that every case passes. In particular, it records failures caused by differing Task 1 classifications or urgency decisions, invalid model JSON in one case, and missing Task 2 recommended actions or data-quality output in some runs.

### Task 3 tests

`Task-3/tests/test_evaluation.py` tests the evaluation logic without calling either LLM. It covers a perfect and failed Task 1 result, Task 2 risk checks, and aggregate summary calculations.
...
Task 3 includes the evaluation harness, ten predefined evaluation cases, deterministic evaluators, equal-weight scoring, optional Gemini judging, and JSON/Markdown report generation. The harness tests itself successfully. The latest stored evaluation report covers all ten cases: 3 passed, 7 failed, with an average deterministic score of `0.680`. This means the three task implementations and their evaluation tooling are present, while some model outputs still need improvement to satisfy every evaluation case.
